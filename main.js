...
//   if (useMobile) throw new Error('Cannot use a pairing code with the Mobile API');
...
function purgeOldFiles() {
  const directories = ['./AuthSession/', './jadibts/'];
...
async function connectionUpdate(update) {
  ...
  if (connection == 'open') {
    console.log(chalk.yellow('[ ℹ️ ] Successfully connected.'));
  }
  ...
  if (connection === 'close') {
    if (reason === DisconnectReason.badSession) {
      ...
      conn.logger.info('[ ⚠ ] Restart necessary, restart the server if it has any problems.');
      await global.reloadHandler(true).catch(console.error);
